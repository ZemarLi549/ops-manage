...
# resp = requests.post(url_hosts, headers=headers, data=json.dumps({'post_data':data}))
# try:
#     es_resp = resp.json().get('data')
#     print(es_resp)
# except:
...
posturl = f"{OPS_DOMAIN}/ops-manage/v1/api/log/gateconfig"
...
def es_search_domain(source_dict):
    ...
    es_index_config = get_index_config(datasource_id, 'nginxlog')
    component_dict = {}
