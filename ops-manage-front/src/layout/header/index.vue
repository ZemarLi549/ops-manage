    ...
    <Space :size="20">
      <slot>
        <Space :size="20">
          ...
          <Breadcrumb style="white-space: nowrap">
...
<script lang="tsx" setup>
  ...
  const doLogout = () => {
    Modal.confirm({
      ...
      centered: true,
      onOk: async () => {
        const ssoTicket = Storage.get('ssoTicket');
        ...
        //
        // window.location.href = `https://ssoqxb.iflytek.com:8443/sso/login?service=http://127.0.0.1:8098/login?ssoQuery=${route.fullPath}`;
        // 重定向到 SSO 登录地址，并带上当前页面作为重定向参数
        // window.location.href = `https://ssoqxb.iflytek.com:8443/sso/logout?service=http://127.0.0.1:8098/login?redirect=${routeQuery}`;
        // window.location.href = `https://ssoqxb.iflytek.com:8443/sso/login?service=http://127.0.0.1:8098/login?redirect=${routeQuery}`;
        if (ssoTicket) {
          window.location.href = `${process.env.VUE_APP_SSO_LOGOUT}#/login?redirect=${routeQuery}`;
        } else {
          router.replace({
            name: LOGIN_NAME,
            query: {
