  ...
  <div>
    <div style="margin-bottom: 16px">
      <a-collapse v-model:activeKey="activeKey">
        <a-collapse-panel key="1" header="说明">
          <p>域名接口过滤项，忽略的uri，可正则匹配，组件和域名如果写all,即针对所有有效！</p>
        </a-collapse-panel>
      </a-collapse>
    </div>
    <div style="margin-bottom: 16px">
      <a-space>
        <a-button v-if="$auth('log.gatepass.add')" type="primary" @click="addNewData"
          >新增</a-button
        >
        <a-button
          v-if="$auth('log.gatepass.delete')"
          type="primary"
          danger
    ...
    >
      <template #bodyCell="{ column, record }">
        <template v-if="column.dataIndex === 'action'">
          <span>
            <a v-if="$auth('log.gatepass.update')" class="a-class" @click="rowEdit(record)">编辑</a>
          </span>
        </template>
        <template v-else>
          <span>
    ...
    >
      ...
      >
        ...
        <a-form-item label="忽略的uri" name="uri">
          <a-input
            v-model:value="formState.uri"
            allow-clear
            placeholder="请输入uri正则，例如：/api/login*"
          />
        </a-form-item>
...
<script>
  ...
  import { getDataListByPage, createData, deleteData, updateData } from '@/api/log/gatepass';
