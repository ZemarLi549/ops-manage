...
<script setup lang="ts">
  ...
  import { alarmLocateData } from '@/api/alarm/identity';
  ...
  const activation = computed(() => {
    return (content) => {
      ...
      if (lowerContent.includes('resolved'.toLowerCase())) {
        return { color: 'green' };
      } else if (lowerContent.includes('忽略'.toLowerCase())) {
        return { color: 'gray' };
      } else if (lowerContent.includes('灾难'.toLowerCase())) {
        return { color: '#ff0808' };
      } else if (lowerContent.includes('紧急'.toLowerCase())) {
        return { color: '#8B0000' };
      } else if (lowerContent.includes('严重'.toLowerCase())) {
        return { color: '#FFA500' };
      } else if (lowerContent.includes('一般'.toLowerCase())) {
        return { color: '#D3D3D3' };
      } else {
        return { color: '#D3D3D3' };
      }
  ...
  const initLocate = async () => {
    ...
    state.logDataList = [];
    state.pullLoading = true;
    await alarmLocateData(requestData).then((res) => {
      state.pullLoading = false;
      if (res.data?.prev) {
      ...
      state.logDataList.push({
        content: `${separation}\n[${props.record.message.startsAt}] [${props.record.message.status}] [${props.record.message.execution}] [${props.record.message.severity}] [指纹ID${props.record.message.alarm_id}] [告警ID${props.record.message.srmid}] [${props.record.message.labels}] 【${props.record.message.alarm_summary}】${props.record.message.alarm_desc}`,
        search_after: props.record.search_after,
      });
      if (res.data?.next) {
